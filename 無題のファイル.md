テキストデータはdataフォルダに上書きするロジックでいこうと思います。
C:\Users\sem3171\PC_production_plan\data


zp58:部材未払いだし一覧
zp138:引当一覧。入庫考慮されているため、別途実在庫での引当と過不足を算出、9万データ弱
zp02:生産表。完成済みも含む、ステータスで識別
ZP51N:PC製造の工程表

t_zp138引当：zp138の加工結果、引当と過不足を算出。PC外注は入っていない


部材不足関連追加
PC生産表に部材不足の識別を追加する。gui生産計画一覧の明細とフィルタ追加
生産表には識別のみ、別途不足の一覧を提示。
計画開始日を超過と計画終了日を超過一覧を各々提示
その他良案あればお願いします！

t_zp138引当作成のスクリプト追加＆改修
sqlite格納は基本全データ
不足一覧参考access
`SELECT ZP02.MRP管理者, ZP02.MRP管理者グループ, zp58.計画開始, zp58.計画終了, ZP02.指図番号, zp58.品目, zp58.品目テキスト, zp58.構成品目, zp58.構成品目テキスト, zp58.所要量, t_zp138引当.引当, t_zp138引当.過不足
FROM (zp58 LEFT JOIN t_zp138引当 ON (zp58.入出庫明細 = t_zp138引当.itm) AND (zp58.入出庫予定 = t_zp138引当.入出庫予定)) LEFT JOIN ZP02 ON zp58.指図／ネットワーク = ZP02.指図番号
WHERE (((ZP02.MRP管理者)="PC") AND ((ZP02.MRP管理者グループ)<"4"));`

path'\\\SAPIF01\host\'
\\\SAPIF01\host\ZP58\ZP58.txt
\\\SAPIF01\host\ZP138\ZP138.txt
\\\SAPIF01\host\ZP02\ZP02.TXT
\\\SAPIF01\host\ZP51N\ZP51N.TXT
...
INPUT_FILE_PATH = r'\\SAPIF01\host\ZP138\ZP138.txt'
